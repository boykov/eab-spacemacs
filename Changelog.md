...
Rm unused packages, commented settings.
Simplify spacemacs.
Rename `org-mode/lisp` -> `org`.
...
Add `eab/gotify-check`, `eab/gotify-create` and recreate socket on
failed check. Use `compilation-ask-about-save`.
Don't sync symlinks in org-directory rsync.
Add `rg`, `helm-org-rifle`.
Update `dash`, `helm-org`, `helm-descbinds`, `s`, `f`, `helm`,
`helm-core`. 
Don't use `helm-config`, `helm-descbinds` due to conflict with which-key.
...
Add `chronosP`.
Decrease `org-tags-column`.
Init projectile portal command.

## 0.2.10

Improve `eab/ergoemacs-new-empty-buffer`.
Add kairos ssh timeout.
Don't use `notify.homew.keenetic.pro`.
